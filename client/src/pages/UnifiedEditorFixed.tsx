...
import EditorShellLayout, { type EditorSkin } from "@/components/EditorShellLayout";
...
export default function UnifiedEditorFixed() {
  ...
  const [skin, setSkin] = useState<EditorSkin>("graphite");
  ...
  const totalVideoDurationMs = useMemo(
    () => scenes.reduce((sum, s) => sum + (s.duration ?? 0), 0) || loadedTemplate?.durationMs || 0,
  ...
  const buildScenes = useCallback(
    (items: MediaItem[]) => {
      if (loadedTemplate)
        return applyTemplateToMedia(loadedTemplate).scenes.map((scene, index) => {
    ...
    img.onload = () => {
      ...
      stickers.forEach((s) => {
        ...
        si.onload = () => {
          ...
          ctx.drawImage(si, -size / 2, -size / 2, size, size);
          ctx.restore();
        };
        si.src = stickerToDataUrl(def);
      });
    };
  ...
  const handleMediaFiles = useCallback((files: FileList | null) => {
    ...
    for (let i = 0; i < files.length; i += 1) {
      ...
      next.push({ id: uid(), type: isVideo ? "video" : "image", fileName: f.name, objectUrl: URL.createObjectURL(f) });
    }
    if (!next.length) {
  ...
  const handlePrev = () => {
    ...
    setSelectedMediaIndex((i) => (i > 0 ? i - 1 : mediaItems.length - 1));
    setSelectedSceneIndex((i) => (i > 0 ? i - 1 : Math.max(0, scenes.length - 1)));
    setIsVideoPaused(true);
  };
  const handleNext = () => {
    if (!mediaItems.length) return;
    setSelectedMediaIndex((i) => (i < mediaItems.length - 1 ? i + 1 : 0));
    setSelectedSceneIndex((i) => (i < scenes.length - 1 ? i + 1 : 0));
    setIsVideoPaused(true);
  };
  const handlePlayPause = () => {
    const v = videoRef.current;
    if (editorType === "photo" || !v) {
      setIsVideoPaused((p) => !p);
      return;
    }
    if (v.paused) v.play().then(() => setIsVideoPaused(false)).catch(() => toast.error("No se pudo reproducir el video."));
    else {
      v.pause();
      setIsVideoPaused(true);
    }
  ...
  const handleRemoveMediaItem = (id: string) =>
    setMediaItems((prev) => {
      ...
      setSelectedMediaIndex((i) => Math.min(i, Math.max(0, out.length - 1)));
      return out;
    });
  const handleDuplicateSelected = async () => {
    if (!selectedMedia) return;
    try {
      const r = await fetch(selectedMedia.objectUrl);
      const b = await r.blob();
      const copy: MediaItem = { ...selectedMedia, id: uid(), fileName: `copia-${selectedMedia.fileName}`, objectUrl: URL.createObjectURL(b) };
      setMediaItems((prev) => {
        const out = [...prev, copy];
        setSelectedMediaIndex(out.length - 1);
        return out;
      });
    } catch {
  ...
  const handleMoveSelectedClip = (d: -1 | 1) => {
    if (scenes.length <= 1) return;
    const a = selectedSceneIndex;
    const b = a + d;
    if (b < 0 || b >= scenes.length) return;
    setScenes((prev) => {
      const out = [...prev];
      const [m] = out.splice(a, 1);
      out.splice(b, 0, m);
      return out.map((s, i) => ({ ...s, order: i }));
    });
    setSelectedSceneIndex(b);
    setSelectedMediaIndex(sceneToMediaIndex(b));
  };
  ...
  const handleProjectLoaded = (p: PeraCutProject) => {
    setEditorType(p.type);
    setProjectName(p.name);
    if (p.type === "photo" && p.photo) {
      if (p.photo.imageDataUrl) setMediaItems([{ id: uid(), type: "image", fileName: "proyecto-guardado.png", objectUrl: p.photo.imageDataUrl }]);
      setSelectedMediaIndex(0);
      setBrightness(p.photo.brightness ?? 100);
      setContrast(p.photo.contrast ?? 100);
      setSaturation(p.photo.saturation ?? 100);
      setRotation(p.photo.rotation ?? 0);
      setSelectedFilter(p.photo.filter ?? "none");
      setStickers(p.photo.stickers ?? []);
    }
    if (p.type === "video" && p.video) {
      setTransitionType(p.video.transitionType ?? "fade");
      setTransitionDuration(p.video.transitionDuration ?? 500);
      setSlowMotionSpeed(p.video.slowMotionSpeed ?? 1);
      const restored = p.video.scenes.map((s, i) => ({
        id: Number(s.id) || i + 1,
        projectId: 0,
        order: i,
        duration: s.duration,
      ...
      setMediaItems(
        restored
          .filter((s) => !!s.mediaUrl)
          .map((s, i) => ({ id: `restored-${i}-${uid()}`, type: s.mediaType, fileName: `scene-${s.id}`, objectUrl: s.mediaUrl! })),
      );
      setSelectedMediaIndex(0);
    }
    setShowTypeSelector(false);
  ...
  const exportDialogButtonGuard = (e: MouseEvent<HTMLButtonElement>) => {
    if (!isRegistered()) {
      e.preventDefault();
      e.stopPropagation();
      requireRegistration(() => {});
  ...
  const warningBannerNode =
    editorType === "video" && totalVideoDurationMs >= MAX_EXPORT_DURATION_MS ? (
  ...
  const saveDialogProps = {
    ...
    videoScenes: scenes.map((s) => ({ id: String(s.id), imageUrl: s.mediaUrl ?? undefined, duration: s.duration })),
    projectName,
    project: buildProject(),
    onProjectLoaded: handleProjectLoaded,
  } as const;
  const compactButtonClass = "w-full h-8 justify-start text-xs px-2";
  const flowState = { hasTemplate: !!loadedTemplate, hasMedia: mediaItems.length > 0, hasTimeline: scenes.length > 0 };

  const rightSidebarContent = (
    <div className="space-y-3 text-[var(--shell-ink)]">
      <div className="grid grid-cols-3 gap-1 rounded-2xl bg-white/5 p-1 border border-white/10">
        {[{ id: "neon", label: "Neon", filter: "cool" }, { id: "light", label: "Light", filter: "vintage" }, { id: "dusk", label: "Dusk", filter: "warm" }].map((x) => (
          <Button
            key={x.id}
            size="sm"
            variant={selectedFilter === x.filter ? "default" : "outline"}
            className="h-8 text-[11px] px-1 rounded-xl"
      ...
      <div className="grid grid-cols-4 gap-1 rounded-2xl bg-white/5 p-1 border border-white/10">
        {(["9:16", "1:1", "16:9", "4:5"] as const).map((x) => (
          <Button
            key={x}
            size="sm"
            variant={previewAspectRatio === x ? "default" : "outline"}
            className="h-8 text-[10px] px-1 rounded-xl"
      ...
      <div className="grid grid-cols-3 gap-1 rounded-2xl bg-white/5 p-1 border border-white/10">
        {([{ key: "media", label: "Media" }, { key: "clip", label: "Clip" }, { key: "project", label: "Proyecto" }] as const).map((x) => (
          <Button
            key={x.key}
            size="sm"
      ...
      <div className="grid grid-cols-2 gap-2">
        ...
        <Button size="sm" variant="outline" className={compactButtonClass} onClick={() => setRightTab("clip")} disabled={editorType !== "video"}>
          <Volume2 className="w-3.5 h-3.5 mr-1" /> Sonido
        </Button>
        <Button size="sm" variant="outline" className={compactButtonClass} disabled={!selectedMedia} onClick={() => selectedMedia && handleRemoveMediaItem(selectedMedia.id)}>
          <X className="w-3.5 h-3.5 mr-1" /> Borrar
        </Button>
        <Button size="sm" variant="outline" className={compactButtonClass} disabled={!selectedMedia} onClick={handleDuplicateSelected}>
      ...
      <Button size="sm" variant="outline" className={compactButtonClass} onClick={() => requireRegistration(() => toast.success("Listo para compartir"))}>
      ...
      {rightTab === "media" && editorType === "photo" ? (
        <div className="space-y-2 pt-2 border-t border-white/10">
          <div>
            <p className="text-[11px] text-[var(--shell-ink-muted)] mb-1">Brillo {brightness}%</p>
            <Slider value={[brightness]} onValueChange={(v) => setBrightness(v[0])} min={0} max={200} step={1} />
          </div>
          <div>
            <p className="text-[11px] text-[var(--shell-ink-muted)] mb-1">Contraste {contrast}%</p>
            <Slider value={[contrast]} onValueChange={(v) => setContrast(v[0])} min={0} max={200} step={1} />
          </div>
          <div>
            <p className="text-[11px] text-[var(--shell-ink-muted)] mb-1">Saturación {saturation}%</p>
            <Slider value={[saturation]} onValueChange={(v) => setSaturation(v[0])} min={0} max={200} step={1} />
          </div>
          <div>
            <p className="text-[11px] text-[var(--shell-ink-muted)] mb-1">Rotación {rotation}°</p>
      ...
      {rightTab === "clip" && editorType === "video" ? (
        <div className="space-y-2 pt-2 border-t border-white/10">
          <div>
            <p className="text-[11px] text-[var(--shell-ink-muted)] mb-1">Velocidad {slowMotionSpeed.toFixed(2)}x</p>
            <Slider value={[slowMotionSpeed]} onValueChange={(v) => setSlowMotionSpeed(v[0])} min={0.25} max={2} step={0.25} />
          </div>
          <div>
            <p className="text-[11px] text-[var(--shell-ink-muted)] mb-1">Transición</p>
          ...
          <div>
            <p className="text-[11px] text-[var(--shell-ink-muted)] mb-1">Duración transición {transitionDuration}ms</p>
            <Slider value={[transitionDuration]} onValueChange={(v) => setTransitionDuration(v[0])} min={100} max={2000} step={100} />
          </div>
          <MusicPanel defaultMusicTrack={loadedTemplate?.defaultMusicTrack} templateDurationMs={loadedTemplate?.durationMs} />
        </div>
      ) : null}

      {rightTab === "project" ? (
        <div className="pt-2 border-t border-white/10 space-y-2 text-[11px] text-[var(--shell-ink-muted)]">
          <p className="font-semibold text-[var(--shell-ink)]">{projectName}</p>
          <p>{editorType === "photo" ? "Proyecto de foto" : "Proyecto de video"}</p>
          {loadedTemplate ? <p className="text-green-400">Plantilla: {loadedTemplate.styleName} · {loadedTemplate.durationMs / 1000}s · {loadedTemplate.aspectRatio}</p> : null}
            ...
            <div className="grid grid-cols-4 gap-1">
              {STICKERS.map((st) => (
                <button
                  ...
                  title={st.name}
                  className="aspect-square rounded border border-white/10 hover:border-[var(--shell-accent)] flex items-center justify-center bg-white/5"
                >
  ...
  return (
    <>
      <EditorShellLayout
        ...
        onTogglePreviewExpanded={() => setIsPreviewExpanded((v) => !v)}
        ...
        timelineControls={
          <UnifiedEditorTimelineControls
            ...
            onSelectScene={(i) => {
              ...
              setSelectedMediaIndex(sceneToMediaIndex(i));
            }}
            onSelectMedia={setSelectedMediaIndex}
            sceneToMediaIndex={sceneToMediaIndex}
          />
        }
        timelineMediaTrack={
          <UnifiedEditorTimelineMediaTrack
            editorType={editorType}
            mediaItems={mediaItems}
            scenes={scenes}
            selectedMediaIndex={selectedMediaIndex}
        ...
        timelineAudioTrack={<UnifiedEditorTimelineAudioTrack editorType={editorType} />}
      />
