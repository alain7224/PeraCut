...
export default function UnifiedEditorFixed() {
  ...
  const [editorSkin, setEditorSkin] = useState<EditorSkin>(() => {
    const stored = localStorage.getItem("editor-skin") as EditorSkin | null;
    return stored ?? "light";
  });
  ...
  const handleSkinChange = (s: EditorSkin) => {
    setEditorSkin(s);
    localStorage.setItem("editor-skin", s);
  };
